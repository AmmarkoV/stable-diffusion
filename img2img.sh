...
then
 ...
 python scripts/img2img.py --prompt "$@" --init-img upimage.jpg --strength 0.3  --n_samples 2 --seed $SEED --plms --ddim_steps 100
 mv outputs/img2img-samples/samples/*.png outputs/txt2img-samples/samples/
else
 rm outputs/txt2img-samples/samples/*.png
 rm outputs/img2img-samples/samples/*.png
 rm outputs/img2img-samples-laion400m/samples/*.png
 python scripts/img2img.py --prompt "$@" --init-img upimage.jpg --n_samples 2 --seed $SEED --plms --ddim_steps 100 --laion400m 
